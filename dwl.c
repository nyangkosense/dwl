...
drawbar(Monitor *m)
{
    ...
    if (m == selmon) {
        ...
        for (size_t i = 0; i < LENGTH(custom_scripts); i++) {
            ...
            if (i > 0) {
                drwl_text(m->drw, curr_x, 0, TEXTW(m, " "), m->b.height, 0, " ", 0);
                curr_x += TEXTW(m, " ");
            }
            
            int width = TEXTW(m, custom_scripts[i].output);
            drwl_text(m->drw, curr_x, 0, width, m->b.height, 0, custom_scripts[i].output, 0);
            curr_x += width;
        }
    }

    /* Get active window and urgent flags */
    wl_list_for_each(c, &clients, link) {
        if (c->mon != m)
            continue;
        occ |= c->tags;
    ...
    for (i = 0; i < LENGTH(tags); i++) {
        ...
        if (occ & 1 << i)
            drwl_rect(m->drw, x + boxs, boxs, boxw, boxw,
                m == selmon && focustop(m) && focustop(m)->tags & 1 << i,
                urg & 1 << i);
        x += w;
    }
    ...
    if ((w = m->b.width - tw - x) > m->b.height) {
        ...
        if (focused && focused->tags & m->tagset[m->seltags]) {
            drwl_setscheme(m->drw, colors[m == selmon ? SchemeSel : SchemeNorm]);
            const char *title = client_get_title(focused);
            // Zentrierung wie in DWM
            int mid = (m->b.width - TEXTW(m, title)) / 2 - x;
            drwl_text(m->drw, x, 0, w, m->b.height, mid, title, 0);
